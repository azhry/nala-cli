...
2. Read [TOOLING.md](TOOLING.md), resolve the official GitHub CLI, and run its non-interactive authentication and repository-access checks before source edits. Follow the current host's permission policy; do not hard-code a sandbox override or assume a failed check is a false credential rejection. Distinguish permission/network failures from rejected authentication using the observed result.
