...
- For destructive or cross-service live acceptance, use a fresh isolated verifier when the facility, safe account, and task-owned fixture exist. Keep preparation, verifier execution, ledger validation, and tracker/PR publication as separate phases; all live mutations and cleanup must target only the task-owned fixture created for the current run. Do not update verification state or claim completion until the verifier ledger is returned and checked. If delegation is unavailable, record the exact blocker. Model size and reasoning effort do not replace this gate.
