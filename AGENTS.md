...
- Prefer the smallest workflow that satisfies the user's request and the required safety/contract gates. When the user rejects a step, phase, section, or explanation as unnecessary, treat that as a scope correction: remove it rather than defend it, rename it, or preserve it under another label. Keep an extra step only when a specific safety rule or task contract requires it, and state that dependency briefly.
...
- For destructive or cross-service live acceptance, the main agent must spawn one fresh isolated verifier and wait for its report when the facility, safe account, and task-owned fixture exist. All live mutations and cleanup must target only the task-owned fixture created for the current run. If delegation is unavailable, record the exact blocker. Model size and reasoning effort do not replace this gate.
